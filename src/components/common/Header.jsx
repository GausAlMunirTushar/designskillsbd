...
const Header = () => {
  ...
  return (
    <header className={`w-full px-4 py-3 border-b ${currentTheme === 'dark' ? "border-gray-700 bg-gray-900" : "border-gray-200 bg-white"}`}>
      <div className="container mx-auto flex items-center justify-between">
        ...
        {/* Navigation Links */}
        <nav className={`md:flex md:space-x-4 ${isMenuOpen ? "block" : "hidden"} md:block absolute md:static top-16 left-0 w-full ${currentTheme === 'dark' ? "bg-gray-900 border-gray-700" : "bg-white border-gray-200"}`}>
          <ul className="flex flex-col md:flex-row md:space-x-4">
            {navLinks.map(({ href, text, icon }) => (
              <li key={text}>
                <a href={href} className={`flex text-sm py-2 px-4 hover:underline items-center ${currentTheme === 'dark' ? "text-gray-300" : "text-gray-900"}`}>
                  {icon} {text}
                </a>
              </li>
            ))}
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          <button
            ...
            className={`p-2 rounded-md flex items-center ${currentTheme === 'dark' ? "bg-gray-800" : "bg-gray-200"}`}
          >
            {currentTheme === 'dark' ? "🌙" : "🌞"}
          </button>
        </div>
      ...
      <div className={`md:hidden ${isMenuOpen ? "block" : "hidden"} mt-2`}>
        <nav className="flex flex-col space-y-2">
          <ul className="flex flex-col space-y-2">
            {navLinks.map(({ href, text, icon }) => (
              <li key={text}>
                <a href={href} className={`text-sm py-2 px-4 flex items-center ${currentTheme === 'dark' ? "text-gray-300 hover:bg-gray-800" : "text-gray-900 hover:bg-gray-100"}`}>
                  {icon} {text}
                </a>
              </li>
            ))}
